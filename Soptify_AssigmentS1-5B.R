...
# additional dataset to highlight with BLUE regeton
# Creating subset of reggaeton 
genre_reggaeton <- dplyr::filter(by_genre, NewGenre=="Reggaeton")
# Plotting the graph ONLY for reggeton / layer mixing
ggplot() +
  geom_line(data=transform(by_genre), aes (year, popularity, group=NewGenre), 
            alpha=0.6, lwd=0.1, colour="black") +
  geom_line(data=genre_reggaeton, aes (year, popularity, group=NewGenre), 
            lwd=0.5, show.legend=FALSE, color="blue") +
...
# Valence
agg <- aggregate(valence ~ year , data=df_filtred, mean)
df_parallel_coord <- data.frame(t(agg))
df_parallel_coord <- df_parallel_coord[2,]
colnames(df_parallel_coord) <- agg$year
df_parallel_coord$Attribute<-rownames(df_parallel_coord)
...
df_top %>% 
  ggparcoord(
    ...
    #order = "anyClass",
    showPoints = TRUE, 
    title = "Standardize to Min = 0 and Max = 1 - Top songs",
    alphaLines = 0.3
  ) + 
  #scale_color_viridis(discrete=TRUE) +
...
#Highlighting Reggaeton and Latin
df_top %>% 
  ggparcoord(
    columns = c(8,9,12, 13), 
    groupColumn = 2, 
  ...
  scale_color_manual(values=c( "#E8E8E8","blue", "#E8E8E8", "#E8E8E8", "#E8E8E8", "#E8E8E8", "red", "#E8E8E8") ) +
  theme_ipsum(axis_title_size = 0) +
  theme(
    plot.title = element_text(size=10)
  )
...
#-------------------------------------------------------------------------#
#-------Important : create 2 calculated / mean  / Factor------------------#
...
#let's focus on 4 Genres Latin and R&B and Regeton with Country are related
df_cir <- df_filtred %>% 
  filter(year >= 2020 & NewGenre != 'Other' & NewGenre != 'Pop' & NewGenre != 'Jazz' & NewGenre != 'Dance' & 
           NewGenre != 'Rock' & NewGenre != 'Disco' & NewGenre != 'Metal' & popularity >30)

#top = 100 #represents 100 singers up to 3 singles per singer
#df_top <- df_cir[with(df_cir, order(-popularity) ),][1:top,]

df_top <- df_cir
df_top3 <- df_top %>%
  group_by(artists, NewGenre) %>%
  summarize(value = max(popularity, na.rm = TRUE))

#---------------
write.csv(df_top3, "temp.csv")
df_top3 <- read.csv("temp.csv",header=TRUE)
levels(df_top3$NewGenre)
df_top3 <- df_top3[,-1]
#---------------
...
empty_bar <- 3
to_add <- data.frame( matrix(NA, empty_bar*nlevels(data$NewGenre), ncol(data)) )
colnames(to_add) <- colnames(data)
to_add$NewGenre <- rep(levels(data$NewGenre), each=empty_bar)
data <- rbind(data, to_add)
data <- data %>% arrange(NewGenre)
...
# Make the plot
p <- ggplot(data, aes(x=as.factor(id), y=value, fill=NewGenre)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
  
  geom_bar(aes(x=as.factor(id), y=value, fill=NewGenre), stat="identity", alpha=0.5) +
  ...
  coord_polar() + 
  geom_text(data=label_data, aes(x=id, y=value+10, label=artists, hjust=hjust), 
  ...
  geom_text(data=base_data, aes(x = title, y = -18, label=NewGenre), 
            hjust=c(1,1,0,0), colour = "black", alpha=0.8, size=4, fontface="bold", 
            inherit.aes = FALSE) 
p
